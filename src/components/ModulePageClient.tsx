  ...
  return (
    <div className="min-h-dvh bg-[var(--color-surface)] pb-20">
      ...
      <main className="px-5 mt-4">
        <div className="mb-6">
          ...
          <div className="space-y-2 stagger-children">
            {mod.fiches.map((fiche, i) => {
              ...
              return (
                <Link key={fiche.id} href={`/module/${mod.id}/fiche/${fiche.id}`} className="block">
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2 stagger-children">
            {mod.activites.map((act) => {
              ...
              return (
                <Link key={act.id} href={`/module/${mod.id}/activite/${act.id}`} className="block">
                  <div
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0" style={{ backgroundColor: mod.color + "20" }}>
                      {act.type === "quiz" ? "❓" : act.type === "classement" ? "📊" : "🏆"}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-[var(--color-navy)]">{act.title}</p>
                      <p className="text-xs text-[var(--color-text-secondary)] mt-0.5 line-clamp-1">{act.description}</p>
                      {isDone && score !== undefined && (
                        <p className="text-xs font-bold mt-1" style={{ color: mod.color }}>Score : {score}%</p>
                      )}
                    </div>
                    <svg className="w-4 h-4 text-gray-300 shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" />
        ...
        <div className="rounded-2xl p-4 text-center border" style={{ backgroundColor: mod.colorLight, borderColor: mod.color + "20" }}>
          <p className="text-3xl mb-1">{mod.badgeIcon}</p>
          <p className="text-sm font-bold" style={{ color: mod.color }}>Badge : {mod.badgeName}</p>
